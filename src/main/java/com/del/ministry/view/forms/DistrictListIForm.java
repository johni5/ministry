...
public class DistrictListIForm extends ObservableIPanel implements Observer {
    ...
    private void renameDistrict() {
        if (districtNumbersF.getSelectedIndex() > -1) {
            ...
            if (!StringUtil.isTrimmedEmpty(number)) {
                ...
                int answer = JOptionPane.showConfirmDialog(Launcher.mainFrame,
    ...
    private void deleteDistrict() {
        ...
        if (selectedItem != null) {
            ...
            try {
                int answer = JOptionPane.showConfirmDialog(Launcher.mainFrame, String.format("Удалить участок %s?", number),
                        "Удаление", JOptionPane.YES_NO_OPTION);
                if (answer == JOptionPane.YES_OPTION) {
                    ServiceManager.getInstance().deleteDistrict(selectedItem.getDistrict().getId());
                    Launcher.mainFrame.setStatusText(String.format("Участок %s был удален", number));
                    initDistrictList();
                    initAddressList();
                    Launcher.mainFrame.initLeftSideTree();
                }
                ...
                Launcher.mainFrame.setStatusError(String.format("Ошибка при удалении участка %s", number), e1);
            }
        }
    }
    ...
    private void editEvent(DistrictAddress da) {
        JDialog i = new JDialog(Launcher.mainFrame, "Редактировать тип адреса", Dialog.ModalityType.APPLICATION_MODAL);
        try {
            ...
            saveBtn.addActionListener(e1 -> {
                ...
                if (si != null) {
                    ...
                    try {
                        ...
                        Launcher.mainFrame.setStatusError("Ошибка сохранения типа адреса", e);
                    }
                }
            });
            ...
            Launcher.mainFrame.setStatusError("Ошибка редактирования типа адреса", e1);
        }
