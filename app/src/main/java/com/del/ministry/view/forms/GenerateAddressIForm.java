...
public class GenerateAddressIForm extends ObservableIFrame {
    ...
    private void generate() {
        ...
        if (ListUtil.isEmpty(areas)) {
            getMainFrame().setStatusError("Укажите районы!");
            return;
        }
        if (ListUtil.isEmpty(bTypes)) {
            getMainFrame().setStatusError("Укажите типы адресов!");
            return;
        }
        ...
        try {
            ...
            if (ListUtil.isEmpty(readyBuildings)) {
                getMainFrame().setStatusError("Нет свободных адресов!");
                return;
            }
...
//                        System.out.println(ready + " -> " + building + " -> " + rndDoor + "(" + doorsCounter.get(building.getId()).get() + ")");
            ...
            if (ready > 0) {
                getMainFrame().setStatusText("К участку было привязано адресов: " + ready);
                getMainFrame().initLeftSideTree();
                notifyObservers();
                initBuildingsCount();
            } else getMainFrame().setStatusError("Не был привязан ни один адрес");

        } catch (Exception e) {
            getMainFrame().setStatusError("Ошибка получения/сохранения данных!", e);
        }
