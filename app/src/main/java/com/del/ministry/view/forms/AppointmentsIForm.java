...
public class AppointmentsIForm extends ObservableIPanel {
    ...
    public AppointmentsIForm() {
        ...
        takeBtn.addActionListener(e -> {
            try {
                ...
                initTable();
                getMainFrame().setStatusText("Возвещатель " + publisher.getFIO() + " теперь обрабатывает участок " + district.getNumber());
                getMainFrame().initLeftSideTree();
            } catch (Exception e1) {
                getMainFrame().setStatusError("Не удалось назначить участок", e1);
            }
        });
                ...
                if (DateUtilz.between(completed, appointment.getAssigned(), DateUtilz.today())) {
                    ...
                    if (comment != null) {
                        ...
                        try {
                            ...
                            initTable();
                            getMainFrame().setStatusText("Возвещатель " + appointment.getPublisher().getFIO() + " сдал участок " + appointment.getDistrict().getNumber());
                            getMainFrame().initLeftSideTree();
                        } catch (CommonException e1) {
                            getMainFrame().setStatusError("Не удалось сдать участок", e1);
                        }
                    }
                } else {
                    getMainFrame().setStatusError(
    ...
    private void initPublishersList() {
        try {
            ...
            getMainFrame().setStatusError("Нет доступа к возвещателям", e);
        }
    }
    ...
    private void initTable() {
        try {
            ...
            getMainFrame().setStatusError("Ошибка доступа к данным о назначениях", e);
        }
    }
        ...
        try {
            ...
            if (!ListUtil.isEmpty(addresses)) {
                ...
                if (JFileChooser.APPROVE_OPTION == result) {
                    ...
                    getMainFrame().setStatusText("Файл сохранен: " + selectedFile.getCanonicalPath());
                }
            } else {
                getMainFrame().setStatusError("Участок не имеет адресов");
            }
        } catch (Exception e) {
            getMainFrame().setStatusError("Ошибка формирования документа", e);
        }
    ...
    public AddressType getDefaultAddressType() {
        if (defaultAddressType == null) {
            try {
                ...
                getMainFrame().setStatusError("Ошибка получения данных", e);
            }
        }
