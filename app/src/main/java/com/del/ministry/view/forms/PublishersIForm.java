...
public class PublishersIForm extends ObservableIPanel {
    ...
    private void addEvent(ActionEvent e) {
        JDialog i = new JDialog(getMainFrame(), "Добавить нового возвещателя", Dialog.ModalityType.APPLICATION_MODAL);
        showPersonalForm(null, i);
    }

    private void editEvent(ActionEvent e) {
        JDialog i = new JDialog(getMainFrame(), "Редактировать возвещателя", Dialog.ModalityType.APPLICATION_MODAL);
    ...
    private void delEvent(ActionEvent e) {
        ...
        try {
            int answer = JOptionPane.showConfirmDialog(getMainFrame(), "Удалить возвещателя " + publisher.getLastName() + "?", "Удаление", JOptionPane.YES_NO_OPTION);
            if (answer == JOptionPane.YES_OPTION) {
                ServiceManager.getInstance().deletePublisher(publisher.getId());
                initTable();
            }
        } catch (Exception e1) {
            getMainFrame().setStatusError("Ошибка при удалении возвещателя", e1);
        }
    }
    ...
    private void showPersonalForm(Publisher publisher, JDialog i) {
        ...
        saveBtn.addActionListener(e -> {
            ...
            try {
                ...
                getMainFrame().setStatusError("Ошибка при сохранении данных возвещателя", e1);
            }
        });
    ...
    private void initTable() {
        try {
            ...
            getMainFrame().setStatusError("Ошибка доступа к данным о возвещателях", e);
        }
